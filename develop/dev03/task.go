...
func quickSort(arr []string, flags flagsCmd) []string {
	...
	if flags.k <= 0 && !flags.n && !flags.M && !flags.h {
		quickSortAlg(res, 0, len(res)-1, flags, &CompareDefault{})
	}
	if flags.k > 0 {
		quickSortAlg(res, 0, len(res)-1, flags, &CompareColumn{})
	}
	if flags.n && flags.k <= 0 {
		quickSortAlg(res, 0, len(res)-1, flags, &CompareInt{})
	}
	if flags.M {
		quickSortAlg(res, 0, len(res)-1, flags, &CompareMonth{})
	}
	if flags.h {
		quickSortAlg(res, 0, len(res)-1, flags, &CompareSuffix{})
	}
...
func (c *CompareDefault) compare(a, b string, f flagsCmd) int {
	a = strings.ToLower(a)
	b = strings.ToLower(b)
	if f.b {
...
func (c *CompareSuffix) compare(a, b string, f flagsCmd) int {
	a = strings.ToLower(a)
	b = strings.ToLower(b)
	if f.b {
		a = strings.TrimSpace(a)
...
func (c *CompareMonth) compare(a, b string, f flagsCmd) int {
	if f.b {
		a = strings.TrimSpace(a)
		b = strings.TrimSpace(b)
	}
...
func (c *CompareColumn) compare(a, b string, f flagsCmd) int {
	if f.b {
		a = strings.TrimSpace(a)
		b = strings.TrimSpace(b)
	}
...
func (c *CompareInt) compare(a, b string, f flagsCmd) int {
	//return 1, if a > b, else -1, 0 if equals
	//convert to ints arr
	if f.b {
		a = strings.TrimSpace(a)
		b = strings.TrimSpace(b)
